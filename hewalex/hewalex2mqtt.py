        ...
        if sh["FNC"] == 0x50:
            ...
            for item in mp.items():
                ...
                if isinstance(item[1], dict):
                    ...
                    # als compacte JSON-array publiceren (index = uur),
                    # zodat het ruim binnen de 255-tekenlimiet van een
                    # MQTT text-entiteit in Home Assistant blijft.
                    val = json.dumps([bool(item[1].get(h, False)) for h in range(24)], separators=(',', ':'))
                else:
                    val = str(item[1])
...
if __name__ == "__main__":
    ...
    # printZPSMqttTopics()
    start_mqtt()
    device_readregisters_enqueue()import os
import threading
#import configparser
import serial
from hewalex_geco.devices import PCWU, ZPS
